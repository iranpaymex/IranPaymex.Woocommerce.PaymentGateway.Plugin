...
function Load_Iranpaymex_Gateway()
{
    ...
    if (class_exists('WC_Payment_Gateway') && !class_exists('IRPMX_Gateway') && !function_exists('Load_Iranpaymex_Gateway')) {


        add_filter('woocommerce_payment_gateways', 'Load_Iranpaymex_Gateway');

        function Load_Iranpaymex_Gateway($methods)
        {
            $methods[] = 'IRPMX_Gateway';
            return $methods;
        }


        class IRPMX_Gateway extends WC_Payment_Gateway
        {

            public function __construct()
            {

                $this->id = 'IRPMX_Gateway';
                ...
                $this->icon = apply_filters('IRPMX_Gateway_logo', WP_PLUGIN_URL . "/" . plugin_basename(dirname(__FILE__)) . '/assets/images/logo.png');
            ...
            public function Send_to_Iranpaymex_Gateway($order_id)
            {
                ...
                $currency = apply_filters('IRPMX_Gateway_Currency', $currency, $order_id);
                ...
                $form = apply_filters('IRPMX_Gateway_Form', $form, $order_id, $woocommerce);

                do_action('IRPMX_Gateway_Gateway_Before_Form', $order_id, $woocommerce);
                echo $form;
                do_action('IRPMX_Gateway_Gateway_After_Form', $order_id, $woocommerce);
                ...
                $CallbackUrl = add_query_arg('wc_order', $order_id, WC()->api_request_url('IRPMX_Gateway'));
                ...
                $Description = apply_filters('IRPMX_Gateway_Description', $Description, $order_id);
                ...
                $Mobile = apply_filters('IRPMX_Gateway_Mobile', $Mobile, $order_id);
                $Email = apply_filters('IRPMX_Gateway_Email', $Email, $order_id);                
                do_action('IRPMX_Gateway_Gateway_Payment', $order_id, $Description, $Mobile);
                ...
                if (!empty($Message) && $Message) {
                    ...
                    $Note = apply_filters('IRPMX_Gateway_Send_to_Gateway_Failed_Note', $Note, $order_id, $Fault);
                    $order->add_order_note($Note);


                    $Notice = sprintf(__('در هنگام اتصال به بانک خطای زیر رخ داده است : <br/>%s', 'woocommerce'), $Message);
                    $Notice = apply_filters('IRPMX_Gateway_Send_to_Gateway_Failed_Notice', $Notice, $order_id, $Fault);
                    if ($Notice)
                        wc_add_notice($Notice, 'error');

                    do_action('IRPMX_Gateway_Send_to_Gateway_Failed', $order_id, $Fault);
                }
            }
            ...
            public function Return_from_Iranpaymex_Gateway()
            {
                ...
                if ($order_id) {
                    ...
                    if ($_GET['secure'] == $hash) {
                        ...
                        if ($order->status != 'completed') {
                            ...
                            if ($Status == 'completed' && isset($Transaction_ID) && $Transaction_ID != 0) {
                                ...
                                $Note = apply_filters('IRPMX_Gateway_Return_from_Gateway_Success_Note', $Note, $order_id, $Transaction_ID, $verify_card_no, $verify_ref_num);
                                if ($Note)
                                ...
                                $Notice = apply_filters('IRPMX_Gateway_Return_from_Gateway_Success_Notice', $Notice, $order_id, $Transaction_ID);
                                if ($Notice)
                                    wc_add_notice($Notice, 'success');

                                do_action('IRPMX_Gateway_Return_from_Gateway_Success', $order_id, $Transaction_ID);
                                ...
                                $Note = apply_filters('IRPMX_Gateway_Return_from_Gateway_Failed_Note', $Note, $order_id, $Transaction_ID, $Fault);
                                if ($Note)
                                    $order->add_order_note($Note, 1);
                                ...
                                $Notice = apply_filters('IRPMX_Gateway_Return_from_Gateway_Failed_Notice', $Notice, $order_id, $Transaction_ID, $Fault);
                                if ($Notice)
                                    wc_add_notice($Notice, 'error');

                                do_action('IRPMX_Gateway_Return_from_Gateway_Failed', $order_id, $Transaction_ID, $Fault);
                            ...
                            $Notice = apply_filters('IRPMX_Gateway_Return_from_Gateway_ReSuccess_Notice', $Notice, $order_id, $Transaction_ID);
                            if ($Notice)
                                wc_add_notice($Notice, 'success');


                            do_action('IRPMX_Gateway_Return_from_Gateway_ReSuccess', $order_id, $Transaction_ID);
                    ...
                    $Notice = apply_filters('IRPMX_Gateway_Return_from_Gateway_No_Order_ID_Notice', $Notice, $order_id, $Fault);
                    if ($Notice)
                        wc_add_notice($Notice, 'error');

                    do_action('IRPMX_Gateway_Return_from_Gateway_No_Order_ID', $order_id, $Transaction_ID, $Fault);
